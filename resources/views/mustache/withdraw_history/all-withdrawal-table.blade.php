<script id="mustache-table" type="text/x-handlebars-template">
	...
	<table class="table table-striped">
        ...
        <tr>
            ...
            <td class="text-wrap">{{this.transaction.updated_user.full_name}}</td>
            ...
            <td>
                {{#ifCond this.transaction.type 'bulk_withdrawal' }}
                    {{#ifCond this.transaction.fee null }}
                        0.00
                    {{ else }}
                        {{hFormatNumber this.transaction.fee}}
                    {{/ifCond}}
                {{ else }}
            ...
            <td class="text-wrap"><a href='/uploads/withdrawals/{{this.import_file}}' download>{{this.import_file}}</a></td>
            ...
            <td class="text-center">
                <div class="dropdown">
                    ...
                    <form action="withdraw/destroy/{{this.id}}" method="post">
                        @endverbatim
                        @csrf
                        @verbatim
                        <input name="_method" type="hidden" value="DELETE">
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <button data-href="withdraw/edit/{{this.id}}" data-title="Edit Withdrawal" data-fullscreen="true" class="dropdown-item dropdown-edit ajax-modal"><i class="mdi mdi-pencil"></i>Edit</button>
