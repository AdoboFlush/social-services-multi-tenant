<script id="mustache-table" type="text/x-handlebars-template">
	...
	<table class="table table-striped">
        <tr>
            <th class="text-wrap">
                <div class="form-check form-all-checkbox">
                    <input class="form-check-input withdrawal-checkbox" type="checkbox" onClick="toggleAllCheckbox(this)" id="toggleAllCheckbox">
                </div>
            </th>
            ...
            {{#each items}}
            <tr>
            {{#ifCond this.status 'applying' }}
            <td class="text-wrap">
                <div class="form-check">
                    <input class="form-check-input withdrawal-checkbox custom-checkbox" type="checkbox" name="wire_transfer_id" id="{{ this.id }}" onClick="individualCheck(this)">
                </div>
            </td>
            {{ else }}
                <td></td>
            ...
            <td class="text-wrap">{{this.bank_branch_code}}</td>
            <td class="text-wrap text-break">{{this.customer_iban}}</td>
            <td class="text-wrap text-break">{{this.customer_name}}</td>
            <td class="text-wrap">
                {{#ifCond this.transaction.type 'bulk_withdrawal' }}
                    {{hFormatNumber this.transaction.amount }}
                {{ else }}
                    {{hFormatNumber this.amount }}
                {{/ifCond}}
            </td>
