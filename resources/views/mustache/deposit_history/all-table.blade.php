<script id="mustache-table" type="text/x-handlebars-template">
	...
	<table class="table table-striped">
        ...
        <tr>
            ...
            <td class="text-wrap">{{hFormatDate this.updated_at}}</td>
            <td class="text-wrap">
            ...
            <td class="text-wrap">{{this.type}}</td>
            ...
            <td class="text-wrap">{{this.bank_name}}</td>
            <td class="text-wrap">{{this.transaction_number}}</td>
            <td class="text-wrap">
                {{#ifCond this.method 'wire_transfer' }}
                    <a href='/uploads/receipts/{{this.import_file}}' download>{{this.import_file}}</a>
                {{ else }}
                    <a href='/uploads/bulk_requests/{{this.import_file}}' download>{{this.import_file}}</a>
                {{/ifCond}}
            </td>
            ...
            <td class="text-center">
                <div class="dropdown">
                    ...
                    <form action="/admin/deposit/destroy/{{this.id}}" method="post">
                        @endverbatim
                        @csrf
                        @verbatim
                        <input name="_method" type="hidden" value="DELETE">
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <button data-href="/admin/deposit/edit/{{this.id}}" data-title="Update Deposit" class="dropdown-item dropdown-edit ajax-modal"><i class="mdi mdi-pencil"></i>Edit</button>
                            <button data-href="/admin/deposit/show/{{this.id}}" data-title="View Deposit" class="dropdown-item dropdown-view ajax-modal"><i class="mdi mdi-eye"></i>View</button>
                            <button class="btn-remove dropdown-item" type="submit" data-ajax="/admin/deposit/destroy/verify/{{this.id}}"><i class="mdi mdi-delete"></i>Delete</button>
                        </div>
                    </form>
                </div>
            </td>
