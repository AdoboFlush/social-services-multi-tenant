			...
			<div class="card no-export">
				<div class="card-body">
					<h4 class="card-title"><span class="panel-title">{{ _lang('Account Types') }}</span>
						<button class="btn btn-primary btn-sm float-right ajax-modal" data-title="{{ _lang('Create Account Type') }}" data-href="{{ route('account_types.create', [], false) }}">{{ _lang('Add New') }}</button>
					</h4>
									...
									<div class="dropdown">
										...
										<form action="{{ action('AccountTypeController@destroy', $accounttype['id']) }}" method="post">
											...
											<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
												<button data-href="{{ action('AccountTypeController@edit', $accounttype['id']) }}" data-title="{{ _lang('Update Account Type') }}" class="dropdown-item dropdown-edit ajax-modal"><i class="mdi mdi-pencil"></i> {{ _lang('Edit') }}</button>
