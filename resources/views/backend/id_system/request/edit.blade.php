...
@section('card-header')
<h3 class="card-title">Edit a Request (ID Number : {{ $data->id_number }})</h3>
@endsection

@section('tab-content')

<form method="post" style="width:100%;" autocomplete="off" action="{{ url('id_system/requests/update') }}" enctype="multipart/form-data">
    
    <input type="hidden" name="id_request_id" id="id_request_id" value="{{ $data->id }}" />
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="member_id" id="member_id" readonly>
                            <option value="{{$data->member->id}}" data-full-name="{{$data->member->full_name}}" selected>
                                {{$data->member->account_number." - ".$data->member->full_name}}
                            </option>
                        </select>
                        <span class="err-message">{{ _lang('Member is required') }}</span>
                    </div>

                    <div class="form-group">
                        <label class="control-label">{{ _lang('Name will appear on ID') }}</label>
                        <input type="text" class="form-control" name="name_on_id" id="name_on_id" value="{{ $data->name_on_id }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="template_id" required>
                            ...
                            <option value="{{$template->id}}" {{ $data->template_id == $template->id ? 'selected="selected"' : ''}}>{{$template->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="remarks" id="remarks" value="{{ old('remarks') }}">{{ $data->remarks }}</textarea>
            ...
            <div class="card">
                <div class="card-body">
                    <div id="results" style="border-style:dashed;width:100%;height:auto;padding:5px;min-height:240px;background-color:#d9d9d9;">
                        <img id="snapshot-image" {{ !empty($data->profile_pic) ? 'src=/uploads/profile/'.$data->profile_pic : '' }} width="100%" height="auto" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div id="captured-signature" style="border-style: dashed; width:100%; min-height:240px;background-color:#d9d9d9;">
                        <img id="signature-image" {{ !empty($data->signature) ? 'src=/uploads/profile/'.$data->signature : '' }} width="100%" height="auto" />
                    </div>
                </div>
            </div> 
        </div>
...
$('#member_id').on('change', function(e){
    let full_name = $(this).find('option:selected').data('full-name');
    $('#name_on_id').val(full_name);
});

$('#remove_profile_pic').on('click', function(e){
	$(".image-tag").val("");
	$('#snapshot-image').attr('src', "");
	$("#profile_pic").val("deleted");
});

$('#remove_signature').on('click', function(e){
	$('#signature-image').attr('src', "");
	$("#signature").val("deleted");
