...
@section('card-header')
<h3 class="card-title">Edit a Template</h3>
@endsection
...
$properties = json_decode($template->properties_json, true);
$front_image = isset($properties['front']['bg']) ? $properties['front']['bg'] : 'id_canvass.png';
$back_image = isset($properties['back']['bg']) ? $properties['back']['bg']: 'id_canvass.png';

@endphp

<form method="post" class="validate" style="width:100%;" autocomplete="off" action="{{ url('id_system/templates/update') }}" enctype="multipart/form-data">

    <input type="hidden" name="bg_front" id="bg_front" value="" />
    <input type="hidden" name="bg_back" id="bg_back" value="" />
    <input type="hidden" name="data_id" id="data_id" value="{{ $template->id }}" />
    <input type="hidden" name="old_front_pic" id="old_front_pic" value="{{ $front_image }}" />
    <input type="hidden" name="old_back_pic" id="old_back_pic" value="{{ $back_image }}" />
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="remarks" id="remarks" value="{{ $template->remarks }}" />
                <span class="err-message">{{ _lang('Remarks is required.') }}</span>
            </div>
        </div>

        <div class="col-md-12">
            <span><input type="checkbox" name="allowed_user_create" id="allowed_user_create" class="mr-2"  {{  $template->allowed_user_create == 1 ? 'checked' : '' }}/>Allow Member to Create</span>
            <span class="err-message">{{ _lang('This is required.') }}</span>
        </div>

        <div class="col-md-12">
            <span><input type="checkbox" name="allowed_user_update" id="allowed_user_update" class="mr-2"  {{  $template->allowed_user_update == 1 ? 'checked' : '' }}/>Allow Member to Update</span>
            <span class="err-message">{{ _lang('This is required.') }}</span>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-8 pt-5 mb-5" style="background-color:#d9d9d9; height: 619px; display: flex; justify-content: center;">
                    <div class="id-canvass-front mb-5" style="position: relative;">
                        <img id="id_front_background" src="{{ !empty($front_image) ? asset('uploads/templates/'.$front_image) : asset('images/id_canvass.png') }}" alt="img" style="width:750px;">
                    </div>
                </div>
                <div class="col-md-4">
            ...
            <div class="row">
                <div class="col-md-8 pt-5 mb-5" style="background-color:#d9d9d9; height: 619px; display: flex; justify-content: center;">
                    <div class="id-canvass-back mb-5" style="position: relative;">
                        <img id="id_back_background" src="{{ !empty($back_image) ? asset('uploads/templates/'.$back_image) : asset('images/id_canvass.png') }}" alt="img" style="width:750px;">
                    </div>
                </div>
                <div class="col-md-4">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                <button type="button" id="save" class="btn btn-primary">{{ _lang('Update Template') }}</button>					
            </div>
        </div>
    </div>
...
$(document).ready(function(){
    ...
    let properties_json = '{{ $template->properties_json }}';
    loadProperties(properties_json);
    ...
    function loadProperties(properties_json) {

        var doc = new DOMParser().parseFromString(properties_json, "text/html");
        properties_json = doc.documentElement.textContent;
        let properties_arr = $.parseJSON( properties_json );
        let front_properties = properties_arr.front;
        let back_properties = properties_arr.back;

        // FRONT
        $.each(front_properties, function(field, props){
           
            let add_css = '';
            if(field == 'bg'){
                return 0;
            }
            $.each(props, function(prop, style){
                if(!['profile_pic', 'signature', 'qr'].includes(field) && prop == 'width') {
                    return;
                }
                add_css += prop+':'+style+';';
            });
            ...
            switch(field){
                case 'profile_pic':
                    div_field = '<div class="field-draggable" data-type="image" id="'+field+'" style="z-index:1000; position: absolute; '+add_css+'"><img src="{{ asset('images/faces/face4.jpg') }}" alt="Profile Pic" style="width:100%;"></div>';
                break;
                case 'signature':
                    div_field = '<div class="field-draggable" data-type="image" id="'+field+'" style="z-index:1000; position: absolute; '+add_css+'"><img src="{{ asset('images/sample_signature.png') }}" alt="Profile Pic" style="width:100%;"></div>';
                break;
                case 'qr':
                    div_field = '<div class="field-draggable" data-type="image" id="'+field+'" style="z-index:1000; position: absolute; '+add_css+'"><img src="{{ asset('images/sample_qr.png') }}" alt="Profile Pic" style="width:100%;"></div>';
                break;
                default:
                    div_field = '<div class="field-draggable" data-type="string" id="'+field+'" style="z-index:1000; position: absolute;'+add_css+'">['+field+']</div>';
            }

            if($('#'+field).length <= 0){
                $('.id-canvass-front').append(div_field);
            }
        ...
        // BACK
        $.each(back_properties, function(field, props){
           ...
           if($('#'+field).length <= 0){
               $('.id-canvass-back').append(div_field);
           }
