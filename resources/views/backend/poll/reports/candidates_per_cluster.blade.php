...
$(function() {
    ...
    $('#generate-report').on('click', function(e) {
        ...
        if(electionId && position) {
            ...
            $.getJSON(`{{ route('poll.reports.candidates_per_cluster_data', [], false) }}?election_id=${electionId}&position=${encodeURIComponent(position)}`)
                .done(function(resp) {
                    let data = resp.data || [];
    ...
    $exportBtn.on('click', function() {
        ...
        a.href = url;
        a.download = 'candidates_per_cluster_report_' + (new Date().toISOString().slice(0,10)) + '.csv';
    ...
    function renderTableReport(data, masterCandidates) {
        ...
        let headers = ['Cluster #'].concat(candidateNames).concat(['Total']);
        let rows = [];
        data.forEach(function(cluster) {
            let row = [cluster.name];
            let total = 0;
            masterCandidates.forEach(function(c) {
                let found = cluster.candidates.find(x => x.id == c.id);
    ...
    function renderReport(data, masterCandidates) {
        ...
        for(let i=0; i<data.length; i+=2) {
            ...
            for(let j=0; j<2; j++) {
                if(data[i+j]) {
                    let cluster = data[i+j];
                    let barangay = cluster.brgy ? `<div class='text-muted small text-center mb-1'>Barangay: ${cluster.brgy}</div>` : '';
                    html += `<div class='col-md-6'>
                        <div class='card card-body p-2'>
                            <h6 class='mb-1 text-center'>Cluster ${cluster.name}</h6>
                            ${barangay}
                            <div style='height:250px;'>
                                <canvas id='cluster-chart-${cluster.id}' style='height:200px !important;'></canvas>
                            </div>
                        </div>
                    </div>`;
                }
        ...
        data.forEach(function(cluster) {
            let ctx = document.getElementById('cluster-chart-'+cluster.id).getContext('2d');
            let sortedCandidates = cluster.candidates.slice().sort((a, b) => Number(b.votes) - Number(a.votes));
    ...
    function renderGraphByBrgy(data) {
        let html = '';
        if(!data || !data.length) {
            $('#report-content').html('<div class="alert alert-info">No data available.</div>');
            return;
        }
        // Render two barangay charts per row for compactness
        for(let i=0; i<data.length; i+=2) {
            html += `<div class='row mb-4'>`;
            for(let j=0; j<2; j++) {
                if(data[i+j]) {
                    ...
                    html += `<div class='col-md-6'>
                        <div class='card card-body p-2'>
                            <h6 class='mb-2 text-center'>${brgy.name}</h6>
                            <div style='height:250px;'>
        ...
        $('#report-content').html(html);
        // Use a set of visually appealing, colorblind-friendly colors
        const palette = [
            '#4e79a7', '#f28e2b', '#e15759', '#76b7b2', '#59a14f', '#edc949',
            '#af7aa1', '#ff9da7', '#9c755f', '#bab0ab', '#86bcb6', '#b07aa1'
        ];
        // Render horizontal bar charts using Chart.js
        data.forEach(function(brgy) {
            ...
            let sorted = brgy.candidates.slice().sort((a,b) => b.votes - a.votes);
            let getRank = cid => {
                let idx = sorted.findIndex(c => c.name === cid.name && c.party === cid.party && cid.party_list === cid.party_list);
                if(idx === 0) return '1st';
                if(idx === 1) return '2nd';
                if(idx === 2) return '3rd';
                return (idx+1)+'th';
            };
            let labels = brgy.candidates.map(c => {
                let rank = getRank(c);
                let partyInfo = c.party + (c.party_list ? ' / ' + c.party_list : '');
                return `${rank} - ${c.name} (${partyInfo})`;
            });
            let votes = brgy.candidates.map(c => Number(c.votes));
            // Custom color logic: if candidate name contains 'ATAYDE', use #3d9970
            let colors = brgy.candidates.map((c, idx) => {
                if ((c.name || '').toUpperCase().includes('ATAYDE')) {
                    return '#3d9970';
                }
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    layout: {
                        padding: {
                            right: 40 // Add right padding for datalabels
                        }
                    },
                    plugins: {
                        legend: { display: false },
